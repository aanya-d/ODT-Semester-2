#gyro 13- worked well (wrist tilt)
...
function loop(ts){
  ...
  //
  var rawX, rawY;
  if(Math.abs(gyroY) > DEADZONE){ rawX = gyroY;  }   // tilt right → pointer right
  else                           { rawX = 0; smoothX = 0; }  // stopped → hard zero

  if(Math.abs(gyroX) > DEADZONE){ rawY = -gyroX; }   // tilt up → pointer up (negate)
  else                           { rawY = 0; smoothY = 0; }  // stopped → hard zero
