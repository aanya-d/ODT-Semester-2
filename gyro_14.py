...
/* Debug overlay */
#dbg{position:fixed;bottom:10px;left:10px;background:rgba(0,0,0,.75);border:1px solid #FFE81F33;border-radius:3px;padding:6px 10px;font-size:11px;line-height:1.8;letter-spacing:1px;color:#00FF88;z-index:30;font-family:monospace;pointer-events:none}
</style>
</head>
<body>
<div id="hud">
...
<canvas id="cv"></canvas>
<div id="dbg">
  GX: <span id="d-gx">0.00</span> &deg;/s<br>
  GY: <span id="d-gy">0.00</span> &deg;/s<br>
  Pointer: <span id="d-cx">0</span>, <span id="d-cy">0</span><br>
  <span id="d-cal" style="color:#FFE81F">Calibrating... hold still</span>
</div>
<div id="over">
...
// ══════════════════════════════════════════════════
//  TUNING — all movement controls are here
// ══════════════════════════════════════════════════
var DEADZONE = 3.0;   // deg/s — raise if pointer still creeps at rest
var SPEED    = 0.5;   // px per deg/s — lower = slower pointer
var ALPHA    = 0.20;  // smoothing (0=laggy, 1=instant/jittery)
// ══════════════════════════════════════════════════

var smoothX=0,smoothY=0;

// ── CALIBRATION ────────────────────────────────────────────────────
//  Your sensor has a bias: it outputs a small non-zero value
//  (e.g. GX=+1.8, GY=+1.6) even when perfectly still.
//  That bias is consistent, so we measure it once at startup
//  and subtract it from every reading from then on.
//
//  Hold the controller still for the first 2 seconds after
//  loading — that's the calibration window.
var calSamplesX = [], calSamplesY = [];
var biasX = 0, biasY = 0;
var calibrated = false;
var CAL_SAMPLES = 40;  // collect 40 readings (~1.6 s) then compute bias
// ──────────────────────────────────────────────────────────────────
...
function updateDebug(){
  var gxc = (gyroX - biasX).toFixed(2);
  var gyc = (gyroY - biasY).toFixed(2);
  document.getElementById('d-gx').textContent = gyroX.toFixed(2) + ' (adj:' + gxc + ')';
  document.getElementById('d-gy').textContent = gyroY.toFixed(2) + ' (adj:' + gyc + ')';
  document.getElementById('d-cx').textContent = Math.round(cx);
  document.getElementById('d-cy').textContent = Math.round(cy);
  var calEl = document.getElementById('d-cal');
  if(calibrated){
    calEl.style.color='#00FF88';
    calEl.textContent='CAL OK  bX:'+biasX.toFixed(2)+' bY:'+biasY.toFixed(2);
  } else {
    calEl.style.color='#FFE81F';
    calEl.textContent='Calibrating... '+calSamplesX.length+'/'+CAL_SAMPLES+' (hold still)';
  }
}
...
function loop(ts){
  ...
  // ── POINTER MOVEMENT ──────────────────────────────────────────────
  //
  //  DRIFT FIX: the previous bug was that smoothX/smoothY were blended
  //  toward zero slowly (×ALPHA each frame), so they never actually
  //  reached zero — they kept adding a tiny residual to cx/cy forever.
  //  Fix: when the deadzone kills the raw input, HARD-RESET smooth to 0
  //  immediately. Pointer stops the instant the gyro stops moving.
  //
  //  AXIS MAPPING:
  //    gyroY drives X (left/right).  gyroY > 0 = tilt right → cx goes right (+)
  //    gyroX drives Y (up/down).     gyroX > 0 = tilt down  → cy goes down  (+)
  //    To make UP = up on screen, negate gyroX for rawY.
  //
  //  POINTER MOVEMENT
  //  1. Subtract calibration bias so a resting sensor reads ~0
  //  2. Apply deadzone to kill any leftover noise
  //  3. Hard-reset smooth to 0 when deadzone kills input (stops residual creep)
  //  4. Accumulate into position
  if(!calibrated){ requestAnimationFrame(loop); return; }  // wait for cal

  var adjX = gyroY - biasY;   // bias-corrected lateral axis
  var adjY = gyroX - biasX;   // bias-corrected vertical axis

  var rawX, rawY;
  if(Math.abs(adjX) > DEADZONE){ rawX =  adjX; }
  else                          { rawX = 0; smoothX = 0; }

  if(Math.abs(adjY) > DEADZONE){ rawY = -adjY; }   // negate: tilt up → pointer up
  else                          { rawY = 0; smoothY = 0; }
...
// ---------- GYRO POLLING + CALIBRATION ----------
// First CAL_SAMPLES readings are used to measure the sensor's
// resting bias. Hold the controller still until the debug overlay
// shows "CAL OK". After that, bias is subtracted every frame.
function pollGyro(){
  ...
  x.onload=function(){
    if(x.status===200){
      try{
        ...
        var gx=parseFloat(d.gx)||0;
        var gy=parseFloat(d.gy)||0;
        gyroX=gx;
        gyroY=gy;
        ...
        if(!calibrated){
          calSamplesX.push(gx);
          calSamplesY.push(gy);
          if(calSamplesX.length >= CAL_SAMPLES){
            // Average all samples → that's our resting bias
            var sumX=0, sumY=0;
            for(var i=0;i<calSamplesX.length;i++){ sumX+=calSamplesX[i]; sumY+=calSamplesY[i]; }
            biasX = sumX / calSamplesX.length;
            biasY = sumY / calSamplesY.length;
            calibrated = true;
            // Centre the pointer at screen centre once calibrated
            cx = W/2; cy = H/2;
            smoothX = 0; smoothY = 0;
            console.log('Gyro calibrated. biasX='+biasX.toFixed(3)+' biasY='+biasY.toFixed(3));
          }
        }
      }catch(e){}
    }
...
while True:
    try:
        ...
        if path == '/gyro':
            gx, gy = read_sensor()
            print("GX:{:.2f}  GY:{:.2f}".format(gx, gy))
