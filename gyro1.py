#gyroscope controller claude code 1
import network
import socket
import machine
import struct
...
function mkDrone(){
  ...
  drones.push({x:sx,y:sy,tx:W/2,ty:H/2,spd:0.4+Math.random()*0.6,r:14+Math.random()*8,rot:Math.random()*Math.PI*2,hit:false,alpha:1,age:0});
}
...
function drawDrone(d){
  ...
  ctx.lineWidth=1.5;
  // body
  ctx.beginPath();ctx.ellipse(0,0,d.r*1.2,d.r*0.6,0,0,Math.PI*2);ctx.fill();ctx.stroke();
  // cockpit
  ctx.fillStyle='#ff6622';
  ctx.beginPath();ctx.ellipse(0,-d.r*0.2,d.r*0.35,d.r*0.3,0,0,Math.PI*2);ctx.fill();
  // wings
  ctx.fillStyle='#2a0800';ctx.strokeStyle='#ff4400';
  ctx.beginPath();ctx.moveTo(-d.r*0.3,0);ctx.lineTo(-d.r*1.8,-d.r*0.8);ctx.lineTo(-d.r*1.5,d.r*0.4);ctx.closePath();ctx.fill();ctx.stroke();
  ctx.beginPath();ctx.moveTo(d.r*0.3,0);ctx.lineTo(d.r*1.8,-d.r*0.8);ctx.lineTo(d.r*1.5,d.r*0.4);ctx.closePath();ctx.fill();ctx.stroke();
  // gun
  ctx.strokeStyle='#ff2200';ctx.lineWidth=2;
  ctx.beginPath();ctx.moveTo(-d.r*1.4,d.r*0.3);ctx.lineTo(-d.r*1.8,d.r*0.3);ctx.stroke();
  ctx.beginPath();ctx.moveTo(d.r*1.4,d.r*0.3);ctx.lineTo(d.r*1.8,d.r*0.3);ctx.stroke();
  ctx.globalAlpha=1;
  ctx.restore();
}
...
function loop(ts){
  ...
  // smooth gyro
  smoothX+=(gyroX-smoothX)*ALPHA;
  smoothY+=(gyroY-smoothY)*ALPHA;
  cx=Math.max(30,Math.min(W-30,cx+smoothX*1.8));
  cy=Math.max(30,Math.min(H-30,cy+smoothY*1.8));
  ...
  x.onload=function(){
    if(x.status===200){
      try{
        ...
        gyroX=parseFloat(d.gy)||0;
        gyroY=parseFloat(d.gx)||0;
      }catch(e){}
    }
    setTimeout(pollGyro,60);
  };
  x.onerror=x.ontimeout=function(){setTimeout(pollGyro,200);};
  x.send();
}
...
while True:
    try:
        ...
        if path == '/gyro':
            gx, gy, gz = read_gyro()
            body = '{{"gx":{:.2f},"gy":{:.2f},"gz":{:.2f}}}'.format(gx, gy, gz)
            body_b = body.encode()
