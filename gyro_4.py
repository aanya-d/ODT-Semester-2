#claude gyroscope remote (4) - fixing deadzones and y inversion
...
function loop(ts){
  ...
  // smooth gyro - Y inverted so physical up = screen up
  smoothX+=(gyroX-smoothX)*ALPHA;
  smoothY+=(-gyroY-smoothY)*ALPHA;
  cx+=smoothX*0.7;
  cy+=smoothY*0.7;
  // soft rubber-band pull from edges instead of hard clamp (no deadzone trap)
  var margin=60;
  if(cx<margin) cx+=(margin-cx)*0.12;
  if(cx>W-margin) cx-=(cx-(W-margin))*0.12;
  if(cy<margin) cy+=(margin-cy)*0.12;
  if(cy>H-margin) cy-=(cy-(H-margin))*0.12;
