...
//
//  DEADZONE (deg/s)
//  The MPU-6050 always outputs a tiny non-zero value
//  even when completely still — this is sensor noise.
//  Any gyro reading BELOW this value is forced to 0,
//  so the pointer only moves on a genuine intentional
//  movement. THIS is what stops the top-left drift.
//  → Raise if the pointer still creeps at rest
//  → Lower if small movements feel ignored
var DEADZONE = 4.0;

//  SPEED (pixels per deg/s)
//  How far the pointer moves for each deg/s above
//  the deadzone. Reduced from original 1.4 → 0.8.
//  → Raise for a faster/more sensitive pointer
//  → Lower for a slower/more controlled pointer
var SPEED = 1.0;

//  ALPHA — smoothing factor (0.0 – 1.0)
//  Each frame: smooth = smooth + (raw - smooth) * ALPHA
//  Low ALPHA = heavily smoothed, slight lag
//  High ALPHA = snappy but can feel jittery
var ALPHA = 0.20;
// ══════════════════════════════════════════════════
...
function loop(ts){
  ...
  // ── POINTER MOVEMENT ──────────────────────────────────────────────
  //
  //  The sensor outputs deg/s even at rest (noise ≈ 0.5–2 deg/s).
  //  Without a deadzone, that noise × SPEED × 60fps = continuous drift.
  //
  //  Step 1 — DEADZONE: zero out any reading below the threshold.
  //           Pointer only moves on a real, intentional tilt.
  //
  //  Step 2 — SMOOTHING: blend the new zeroed value into a running
  //           average so movement feels fluid not jittery.
  //
  //  Step 3 — ACCUMULATE: add smoothed velocity × SPEED to position.
  //
  //  AXIS MAPPING (corrected):
  //    gyroY > 0  = sensor tilts RIGHT  → cx increases  (+ gyroY)
  //    gyroY < 0  = sensor tilts LEFT   → cx decreases
  //    gyroX > 0  = sensor tilts DOWN   → cy increases  (+ gyroX)
  //    gyroX < 0  = sensor tilts UP     → cy decreases
  //  Previously rawX used -gyroY which inverted left/right. Fixed below.
  //
  var rawX = Math.abs(gyroY) > DEADZONE ? gyroY : 0;    // LEFT fix: was -gyroY
  var rawY = Math.abs(gyroX) > DEADZONE ? gyroX : 0;    // tilt down = down
