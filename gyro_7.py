#claude code with accelerometer - 7
import network
import socket
import machine
import struct
...
var cx=W/2,cy=H/2;
var gyroX=0,gyroY=0;
var accelPitch=0,accelRoll=0;
// Fused tilt angles in degrees — these are the ground truth pointer position
var tiltX=0,tiltY=0;
// Calibration offset — captured on first good reading so centre = centre
var calX=null,calY=null;
var GYRO_TRUST=0.95;   // how much to trust gyro each frame (rest goes to accel)
var SMOOTH=0.20;       // light smoothing on final pointer position
var SENS=7.0;          // degrees of tilt -> fraction of half-screen
var lastT=null;
var dispX=0,dispY=0;   // smoothed display position
...
function loop(ts){
  ...
  // --- Complementary filter: fuse accel angle + gyro rate -> stable absolute tilt ---
  var now=performance.now();
  var dt=lastT?Math.min((now-lastT)/1000,0.05):0.016; // seconds since last frame, capped
  lastT=now;

  // Gyro integration: tilt += gyroRate * dt  (fast, no drift over short moves)
  tiltX+=gyroX*dt;
  tiltY+=gyroY*dt;

  // Accel correction: pull fused angle toward gravity-derived angle (kills long-term drift)
  // accelPitch/Roll come from pollGyro — they are the absolute tilt from gravity
  tiltX=tiltX*GYRO_TRUST + accelRoll*(1-GYRO_TRUST);
  tiltY=tiltY*GYRO_TRUST + accelPitch*(1-GYRO_TRUST);

  // Calibrate: on first frame lock current angle as "centre"
  if(calX===null){calX=tiltX;calY=tiltY;}

  // Map tilt offset from calibration centre -> screen position
  var targetX=W/2 + (tiltX-calX)/SENS*(W/2);
  var targetY=H/2 + (tiltY-calY)/SENS*(H/2);

  // Light smoothing for display only (keeps pointer stable without adding lag)
  dispX+=( targetX-dispX)*SMOOTH;
  dispY+=( targetY-dispY)*SMOOTH;
  cx=Math.max(20,Math.min(W-20,dispX));
  cy=Math.max(20,Math.min(H-20,dispY));
  ...
  x.onload=function(){
    if(x.status===200){
      try{
        ...
        // Gyro rates (deg/s) — used for fast integration
        gyroX=parseFloat(d.gx)||0;
        gyroY=parseFloat(d.gy)||0;
        // Accel values (g-force) — derive absolute tilt angles from gravity vector
        var ax=parseFloat(d.ax)||0;
        var ay=parseFloat(d.ay)||0;
        var az=parseFloat(d.az)||1;
        // atan2 gives angle in radians, convert to degrees
        // Roll  = tilt left/right,  Pitch = tilt up/down
        accelRoll  = Math.atan2(ay, az) * 180/Math.PI;
        accelPitch = Math.atan2(-ax, Math.sqrt(ay*ay+az*az)) * 180/Math.PI;
      }catch(e){}
    }
    setTimeout(pollGyro,30); // poll every 30ms for snappy response
  };
...
while True:
    try:
        ...
        if path == '/gyro':
            ax, ay, az, gx, gy = read_sensor()
            body = '{{"ax":{:.3f},"ay":{:.3f},"az":{:.3f},"gx":{:.2f},"gy":{:.2f}}}'.format(ax, ay, az, gx, gy)
            body_b = body.encode()
        ...
        try:
            ...
            pass
    #didnt work at all, stuck to the edges only
