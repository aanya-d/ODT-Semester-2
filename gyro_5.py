#claude gyro code 5- fixing all axes and sensitivty 
import network
import socket
import machine
import struct
...
function loop(ts){
  ...
  // Gyro: negate both axes so physical direction matches screen direction
  // Apply deadzone to filter out small tremors/jitter
  ...
  cx+=smoothX*SPEED;
  cy+=smoothY*SPEED;
  // Wide rubber-band margins — full screen always reachable, no stuck edges
  var mx=W*0.08, my=H*0.08;
  if(cx<mx) cx+=(mx-cx)*0.08;
  if(cx>W-mx) cx-=(cx-(W-mx))*0.08;
  if(cy<my) cy+=(my-cy)*0.08;
  if(cy>H-my) cy-=(cy-(H-my))*0.08;
